...
def calculate_ttc(d_rel: float, v_ego: float, v_rel: float) -> float:
  # time-to-collision = distance / relative velocity (in m/s)
  # only when velocity of ego > 0 and relative velocity > 0 (gaining in on lead vehicle)
...
class Track():
  ...
  def update(self, d_rel, y_rel, v_rel, v_lead, measured, v_ego):
    ...
    self.ttc = calculate_ttc(self.dRel, v_ego, self.vRel)
...
class Cluster():
  ...
  def get_RadarState_from_vision(self, lead_msg, v_ego):
    ...
    return {
      ...
      "ttc": float(calculate_ttc(dRel, v_ego, vRel))
    }
